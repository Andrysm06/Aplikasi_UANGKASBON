    ...
    <div className="space-y-8 animate-fade-in pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-dark-500 font-bold uppercase tracking-widest mt-1">Kelola Siapa Saja Yang Bisa Login Ke Sistem</p>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
           <div className="glass-card w-full max-w-md p-0 overflow-hidden shadow-2xl border-white/5">
              <div className="p-6 border-b border-white/5 bg-white/3 flex items-center justify-between">
                 <h3 className="text-xl font-black text-white uppercase tracking-tighter">Tambah Akses Login</h3>
                 <button onClick={() => setModalOpen(false)} className="p-2 hover:bg-white/10 rounded-xl transition-colors"><FiX /></button>
              </div>
              <form onSubmit={handleAddUser} className="p-8 space-y-6">
                 <div>
                    <label className="label uppercase text-[10px] font-black">Nama Lengkap Pengguna</label>
                    <input type="text" className="input-field" value={form.nama} onChange={e => setForm({...form, nama: e.target.value.toUpperCase()})} placeholder="MASUKKAN NAMA PEMEGANG AKUN" required />
                 </div>
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       <label className="label uppercase text-[10px] font-black">Username Login</label>
                       <input type="text" className="input-field font-mono lowercase" value={form.username} onChange={e => setForm({...form, username: e.target.value})} placeholder="username" required />
                    </div>
                    <div>
                       <label className="label uppercase text-[10px] font-black">Kata Sandi (Password)</label>
                 ...
                 <div>
                    <label className="label uppercase text-[10px] font-black">Hak Akses (Role)</label>
                    <select className="input-field" value={form.role} onChange={e => setForm({...form, role: e.target.value})}>
                       <option value="manager">MANAGER (Full Akses)</option>
                       <option value="staff">STAFF (Kasir)</option>
                    </select>
                 </div>
                 <button type="submit" className="w-full h-16 bg-primary-600 text-white rounded-[24px] text-base font-black tracking-[0.2em] shadow-xl shadow-primary-600/20 active:scale-95 transition-all">BUAT AKUN SEKARANG</button>
              </form>
           </div>
        </div>
      )}
